...
for ax, col, q in zip(axes, stdSC_cols, questions_sc):
    ...
    for meanval, col, y, label in zip([mean_T, mean_C, mean_w1], ["magenta", "gold", "orange"], [0.9,0.75, 0.4], ["T", "C", "{w1}"]):
        ax.vlines(
            meanval,
            0,1.8,
            color=col,
        ...
        s = '{s}'
        ax.text(meanval, ax.get_ylim()[1]*y, rf"   $\overline{s}_{label}$:"+f" {meanval:.2f}", ha='left', va='top', color=col)
