...
col_alpha_corrP <- paste0("corrP_alpha_",    questions_sc)
col_alpha_corrS <- paste0("corrS_alpha_",    questions_sc)
...
#| label: H6
# H6 - party
run_H_joint    ("exp_alpha", "party_close", "H6", use_lmer = FALSE, categorical = TRUE)
run_H_joint    ("corrP_alpha",  "party_close", "H6", use_lmer = TRUE,  categorical = TRUE)
```
...
run_H_per_issue("exp_alpha",    "party_close", "H6", use_lmer = FALSE, categorical = TRUE)
run_H_per_issue("corrP_alpha",  "party_close", "H6", use_lmer = TRUE,  categorical = TRUE)
...
# H7 - lr
run_H_joint    ("exp_alpha", "lr", "H7", use_lmer = FALSE)
run_H_joint    ("corrP_alpha",  "lr", "H7", use_lmer = TRUE)
...
```{r}
#| label: H7-per-issue
run_H_per_issue("exp_alpha",    "lr", "H7", use_lmer = FALSE)
run_H_per_issue("corrP_alpha",  "lr", "H7", use_lmer = TRUE)
...
```{r}
#| label: H8
run_H_joint    ("linear_alpha", "age", "H8", use_lmer = FALSE)
run_H_joint    ("corrP_alpha",  "age", "H8", use_lmer = TRUE)
```

And again, per issue:
```{r}
#| label: H8-per-issue
run_H_per_issue("exp_alpha",    "age", "H8", use_lmer = FALSE)
run_H_per_issue("corrP_alpha",  "age", "H8", use_lmer = TRUE)
...
### Exploration 2: Can we at least predict the issue weights based on party, lr and age?
...
df_h9_plot <- df_p |>
  ...
  rename(var_sc = all_of(paste0(variance_type, "_socialCircle_ops")), alpha = all_of(paste0(k, "_alpha"))) |>
  filter(!is.na(alpha), !is.na(var_sc))
...
df_h13_base <- df_diff |>
  ...
  left_join(df_p |> select(id, wave, all_of(col_weights),
                            all_of(col_alpha_lin), all_of(col_alpha_exp), all_of(col_alpha_corrP)),
...
pred_cols <- c("predicted_d_weights", "predicted_d_population_exp",
               ...
               "predicted_d_fit_exp", "predicted_d_corrP", "predicted_d_corrPrel")
...
df_h13 <- df_h13_base |>
  mutate(
    ...
    predicted_d_corrP        = exp_kernel(   rowSums(.d * .a_corrP), exp_param1,    exp_param2),
    predicted_d_corrPrel        = exp_kernel(   rowSums(.d * .a_corrP_rel), exp_param1,    exp_param2)
  ) |>
