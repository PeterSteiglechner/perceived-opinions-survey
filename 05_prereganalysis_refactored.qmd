...
pkgs <- c(
  ...
  "prettycode", "performance", "estimatr", "patchwork"
)
...
print_fit_table <- function(...) {
  ...
  tbl <- data.frame(
    ...
    logLik   = round(sapply(models, \(m) as.numeric(logLik(m))), 1),
    npar     = sapply(models, \(m) length(fixef(m)))
  )
...
df_p <- df_p |>
  mutate(
    ...
    across(all_of(col_var_std), \(x) x^2,
...
between_means <- vapply(seq_len(n), \(i) {
...
# Helper: run H6 for one weight type and print verdict
run_h6 <- function(k, use_lmer = TRUE) {
  alpha_cols <- paste0(k, "_alpha_", questions_sc)
  ...
  if (use_lmer) {
    m_null <- lmer(alpha_q ~ issue + wave + (1 | id),               data = df_h6)
    m_full <- lmer(alpha_q ~ party_close * issue + wave + (1 | id), data = df_h6)
    wt     <- anova(m_null, m_full)
    stat   <- wt$Chisq[2]; df_num <- wt$Df[2]; p <- wt$`Pr(>Chisq)`[2]
    print_fit_table("Null" = m_null, "Full" = m_full)
    print_lrt(sprintf("H6 (%s): party × issue interaction", k),
              stat, df_num, p = p, type = "chi2")
  } else {
    m_null <- lm(alpha_q ~ issue + wave,               data = df_h6)
    m_full <- lm(alpha_q ~ party_close * issue + wave, data = df_h6)
    wt     <- anova(m_null, m_full)
    stat   <- wt$F[2]; df_num <- wt$Df[2]; df_den <- wt$Res.Df[2]; p <- wt$`Pr(>F)`[2]
    # clustered-SE model for coefficient display
    m_full_rob <- lm_robust(alpha_q ~ party_close * issue + wave,
...
run_h6("corrS", use_lmer = TRUE)
run_h6("exp",   use_lmer = FALSE)
```
...
# Per-issue breakdown (both weight types)
run_h6_per_issue <- function(k, use_lmer = TRUE) {
  cat(sprintf("\n-- H6 per issue (%s) --\n", k))

  purrr::walk(questions_sc, \(q) {
    alpha_col <- paste0(k, "_alpha_", q)
    df_q <- df_p |>
      filter(wave %in% c(1, 2)) |>
      select(id, wave, party_close, alpha_q = all_of(alpha_col)) |>
      filter(party_close %in% c(partiesCloseSel, "No party"), !is.na(alpha_q)) |>
    ...
    if (use_lmer) {
      ...
      m      <- lm_robust(alpha_q ~ party_close + wave, data = df_q,
                          clusters = id, se_type = "CR2")
      m_lm   <- lm(alpha_q ~ party_close + wave, data = df_q)
      m_null <- lm(alpha_q ~ wave,               data = df_q)
      wt     <- anova(m_null, m_lm)
      p      <- wt$`Pr(>F)`[2]; f <- wt$F[2]
      cat(sprintf("\n%s  R^2=%.3f  F(%d,%d)=%.2f  p=%.4f %s\n",
                  q, summary(m_lm)$r.squared,
                  wt$Df[2], wt$Res.Df[2], f, p, sig_stars(p)))
    }
  })
}

run_h6_per_issue("corrS", use_lmer = TRUE)
run_h6_per_issue("exp",   use_lmer = FALSE)
```
...
cat("=== H7: Left-right placement predicts issue weights ===\n\n")
...
purrr::walk(questions_sc, \(q) {
  ...
  m  <- lmer(alpha_q ~ lr + wave + (1 | id), data = df_q)
  cf <- lmer_coef(m, "lr")
  cat(sprintf("  %-35s  beta=%.4f  t=%.3f  p=%.4f  %s\n",
              q, cf$beta, cf$t, cf$p2, sig_stars(cf$p2)))
})

# Joint model (LRT)
df_long_h7 <- df_p |>
  filter(wave %in% c(1, 2)) |>
  select(id, wave, lr, starts_with(paste0(k, "_alpha_"))) |>
  pivot_longer(starts_with(paste0(k, "_alpha_")),
               names_to = "issue", values_to = "alpha") |>
  mutate(issue = gsub(paste0(k, "_alpha_"), "", issue), issue = factor(issue)) |>
  filter(!is.na(alpha))

m_full_h7 <- lmer(alpha ~ issue + issue:lr + wave + (1 | id), data = df_long_h7)
m_null_h7 <- lmer(alpha ~ issue + wave + (1 | id),            data = df_long_h7)
wt_h7     <- anova(m_null_h7, m_full_h7)

cat("\n")
print_lrt("H7 joint: lr × issue",
          wt_h7$Chisq[2], wt_h7$Df[2], p = wt_h7$`Pr(>Chisq)`[2], type = "chi2")
...
cat("=== H8: Age predicts issue weights ===\n\n")
...
purrr::walk(questions_sc, \(q) {
  ...
  m  <- lmer(alpha_q ~ age_z + wave + (1 | id), data = df_q, REML = FALSE)
  cf <- lmer_coef(m, "age_z")
  cat(sprintf("  %-35s  beta=%.4f  t=%.3f  p=%.4f  %s\n",
              q, cf$beta, cf$t, cf$p2, sig_stars(cf$p2)))
})
...
# Joint model
df_long_h8 <- df_p |>
  filter(wave %in% c(1, 2), !is.na(age)) |>
  mutate(age_z = as.numeric(scale(age))) |>
  select(id, wave, age_z, starts_with(paste0(k, "_alpha_"))) |>
  pivot_longer(starts_with(paste0(k, "_alpha_")),
               names_to = "issue", values_to = "alpha") |>
  mutate(issue = gsub(paste0(k, "_alpha_"), "", issue), issue = factor(issue)) |>
  filter(!is.na(alpha))

m_full_h8 <- lmer(alpha ~ issue + issue:age_z + wave + (1 | id), data = df_long_h8, REML = FALSE)
m_null_h8 <- lmer(alpha ~ issue + wave + (1 | id),               data = df_long_h8, REML = FALSE)
wt_h8     <- anova(m_null_h8, m_full_h8)

cat("\n")
print_lrt("H8 joint: age × issue",
          wt_h8$Chisq[2], wt_h8$Df[2], p = wt_h8$`Pr(>Chisq)`[2], type = "chi2")
```
...
per_person_r2 <- df_h13 |>
  group_by(id) |>
  summarise(across(all_of(pred_cols), \(x) {
