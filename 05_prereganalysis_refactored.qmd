...
{\color{red} THIS VERSION CALCULATES THE WEIGHTS PURELY BASED ON }
...
\begin{minipage}{\textwidth}
{\color{red}NOTE: I am using all dots to infer the issue weights here. For using just the parties, change the line below to ```fitPartyDots```}
\end{minipage}
...
### Exploration 2: Can we predict the participant-reported issue importance based on party, lr, and age any better?

```{r}
#| label: H6-reported-weights
run_H_joint    ("w", "party_close", "H6 Weights", use_lmer = TRUE)
```

And again, per issue:
```{r}
#| label: H6-reported-weights-per-issue
run_H_per_issue("w",  "party_close", "H6 Weights", use_lmer = TRUE)

```

```{r}
#| label: H7-reported-weights
run_H_joint    ("w", "lr", "H7 Weights", use_lmer = TRUE)
```

And again, per issue:
```{r}
#| label: H7-reported-weights-per-issue
run_H_per_issue("w",  "lr", "H7 Weights", use_lmer = TRUE)

```

```{r}
#| label: H8-reported-weights
run_H_joint    ("w", "age", "H8 Weights", use_lmer = TRUE)
```

And again, per issue:
```{r}
#| label: H8-reported-weights-per-issue
...
df_h13_base <- df_diff |>
  ...
  left_join(df_p |> select(id, wave, all_of(col_weights),
                            all_of(col_alpha_lin), all_of(col_alpha_log), all_of(col_alpha_exp), all_of(col_alpha_corrP)),
...
.a_weights <- as.matrix(df_h13_base[, col_weights])

pred_cols <- c("weights", "weightsrel", "population_exp",
               "population_linear", "population_logistic", "uniform_exp",
               "uniform_linear", "uniform_logistic", "fit_linear",
               "fit_exp", "fit_logistic", "corrP", "corrPrel")
p <- function(prefix, n) {
...
df_h13 <- df_h13_base |>
  mutate(
    ...
    weights                = linear_kernel(rowSums(.d * .a_weights),      p("linear", 1),      p("linear", 2)),
    weightsrel                = linear_kernel(rowSums(.d * .a_weights_rel),      p("linear", 1),      p("linear", 2)),
    corrP                  = linear_kernel(rowSums(.d * .a_corrP),        p("linear", 1),      p("linear", 2)),
    corrPrel               = linear_kernel(rowSums(.d * .a_corrP_rel),    p("linear", 1),      p("linear", 2))
  ) |>
...
```{r}
# df_h13_base <- df_h13_base |>
#   mutate(
#     d_corrP    = rowSums(.d * .a_corrP),
#     d_corrP_rel = rowSums(.d * .a_corrP_rel),
#     d_weights = rowSums(.d * .a_weights),
#     d_weights_rel = rowSums(.d * .a_weights_rel),
#     d_uniform = rowSums(.d * .a_uniform),
#     d_pop_exp = rowSums(.d * .a_pop_exp),
#     d_pop_log = rowSums(.d * .a_pop_log),
#     d_pop_linear = rowSums(.d * .a_pop_lin),
#     d_individ_exp = rowSums(.d * .a_fit_exp),
#     d_individ_log = rowSums(.d * .a_fit_log),
#     d_individ_linear = rowSums(.d * .a_fit_lin),
#   )

# # 2. Fit global linear_kernel params separately for corrP and corrPrel
# #    (generic NLS via optim -- works regardless of linear_kernel's exact form)
# fit_global_linear <- function(d, y, start = c(1, 0)) {
#   obj <- function(par) mean((linear_kernel(d, par[1], par[2]) - y)^2, na.rm = TRUE)
#   optim(start, obj, method = "BFGS")
# }
# fit_global_exp <- function(d, y, start = c(1, 0)) {
#   obj <- function(par) mean((exp_kernel(d, par[1], par[2]) - y)^2, na.rm = TRUE)
#   optim(start, obj, method = "BFGS")
# }
# fit_global_log <- function(d, y, start = c(1, 0)) {
#   obj <- function(par) mean((log_kernel(d, par[1], par[2]) - y)^2, na.rm = TRUE)
#   optim(start, obj, method = "BFGS")
# }

# fit_weights    <- fit_global_linear(df_h13_base$d_weights,     df_h13_base$pixel_dist)
# fit_weightsrel <- fit_global_linear(df_h13_base$d_weights_rel, df_h13_base$pixel_dist)
# fit_corrP    <- fit_global_linear(df_h13_base$d_corrP,     df_h13_base$pixel_dist)
# fit_corrPrel <- fit_global_linear(df_h13_base$d_corrP_rel, df_h13_base$pixel_dist)
# fit_uniform_exp <- fit_global_exp(df_h13_base$d_uniform, df_h13_base$pixel_dist)
# fit_uniform_linear <- fit_global_linear(df_h13_base$d_uniform, df_h13_base$pixel_dist)
# fit_uniform_log <- fit_global_log(df_h13_base$d_uniform, df_h13_base$pixel_dist)
# fit_pop_exp <- fit_global_exp(df_h13_base$d_pop_exp, df_h13_base$pixel_dist)
# fit_pop_linear <- fit_global_linear(df_h13_base$d_pop_linear, df_h13_base$pixel_dist)
# fit_pop_log <- fit_global_log(df_h13_base$d_pop_log, df_h13_base$pixel_dist)
# fit_individ_exp <- fit_global_exp(df_h13_base$d_individ_exp, df_h13_base$pixel_dist)
# fit_individ_linear <- fit_global_linear(df_h13_base$d_individ_linear, df_h13_base$pixel_dist)
# fit_individ_log <- fit_global_log(df_h13_base$d_individ_log, df_h13_base$pixel_dist)


# par_individlog    <- fit_individ_log$par
# par_individexp    <- fit_individ_exp$par
# par_individlinear <- fit_individ_linear$par
# par_poplog    <- fit_pop_log$par
# par_popexp    <- fit_pop_exp$par
# par_poplinear <- fit_pop_linear$par
# par_uniformlog    <- fit_uniform_log$par
# par_uniformexp    <- fit_uniform_exp$par
# par_uniformlinear <- fit_uniform_linear$par
# par_weights    <- fit_weights$par
# par_weightsrel <- fit_weightsrel$par
# par_corrP    <- fit_corrP$par
# par_corrPrel <- fit_corrPrel$par

# # sanity check convergence
# fit_corrP$convergence    # 0 = converged
# fit_corrPrel$convergence

# # 3. Use these fitted (not p("linear", ...)) params in the main mutate
# df_h13 <- df_h13_base |>
#   mutate(
#     uniform_exp          = exp_kernel(rowSums(.d * .a_uniform),    par_uniformexp[1], par_uniformexp[2] ),
#     uniform_linear        = linear_kernel(rowSums(.d * .a_uniform),  par_uniformlinear[1], par_uniformlinear[2]),
#     uniform_logistic      = log_kernel(rowSums(.d * .a_uniform),   par_uniformlog[1], par_uniformlog[2]),
#     population_exp        = exp_kernel(rowSums(.d * .a_pop_exp),      par_popexp[1], par_popexp[2]),
#     population_linear     = linear_kernel(rowSums(.d * .a_pop_lin),   par_poplinear[1], par_poplinear[2]),
#     population_logistic   = log_kernel(rowSums(.d * .a_pop_log),  par_poplog[1], par_poplog[2]),
#     fit_exp                = exp_kernel(rowSums(.d * .a_fit_exp),      par_individexp[1], par_individexp[2]),
#     fit_linear              = linear_kernel(rowSums(.d * .a_fit_lin),  par_individlinear[1], par_individexp[2]),
#     fit_logistic            = log_kernel(rowSums(.d * .a_fit_log),   par_individlog[1], par_individlog[2]),
#     weights                = linear_kernel(rowSums(.d * .a_weights),      par_weights[1], par_weights[2]),
#     weightsrel                = linear_kernel(rowSums(.d * .a_weights_rel),     par_weightsrel[1],    par_weightsrel[2]),
#     corrP                  = linear_kernel(d_corrP,     par_corrP[1],    par_corrP[2]),
#     corrPrel               = linear_kernel(d_corrP_rel, par_corrPrel[1], par_corrPrel[2])
#   ) |>
#   mutate(across(all_of(pred_cols), ~ pixel_dist - .x, .names = "err_{.col}"))
```
...
#### Step 3: Analysis of predictions 
```{r}
#| label: H13-plots
